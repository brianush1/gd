#!/usr/bin/env python3
# Derived from the Erupted Vulkan binding generator.
# Copyright 2016 Alex Parrill and Peter Particle.
# SPDX-License-Identifier: MIT

"""Generate GD's raw D Vulkan bindings from the Khronos Vulkan registry.

Usage: genvulkan.py path/to/Vulkan-Headers/registry outputdir
"""
...
re_long_int = re.compile(r"((?:0[xX][0-9A-Fa-f]+|[0-9]+))ULL")

D_KEYWORDS = { "module", "scope", "version", "function" }
...
FUNCTIONS_HEADER = GENERATED_HEADER + """\
...
class DGenerator( OutputGenerator ):
	...
	def __init__( self, errFile = sys.stderr, warnFile = sys.stderr, diagFile = sys.stderr ):
		...
		self.platformExtensions = {
			...
			"// VK_KHR_win32_surface"            : [ "gd_Win32",                   "public import core.sys.windows.windows;\n" ],
			"// VK_KHR_xlib_surface"             : [ "gd_X11Impl",                 "public import gd.bindings.x11;\n" ],
...
""".format(
			...
			for section in self.TYPE_SECTIONS:
				...
				if contents:
					...
					if section == 'struct':
						if self.opaqueStruct:
							for opaque in self.opaqueStruct:
								if opaque.startswith( "Vk" ):
									continue
								if any( re.search( r"\\b(?:struct|union)\\s+" + re.escape( opaque ) + r"\\s*\\{", declaration ) for declaration in contents ):
									continue
					...
					for content in self.sections[ section ]:
						fileContent += "{1}{0}\n".format( content, "" if content.startswith( "\n" ) else extIndent )
	...
	def genType( self, typeinfo, name, alias = None ):
		...
		if category == "handle":
			...
			underlyingType = typeinfo.elem.find( "type" )
			if underlyingType is not None:
				underlyingType = underlyingType.text
			else:
				underlyingType = alias or typeinfo.elem.attrib.get( "alias" )
			self.appendSection( "bitmask", "alias {0} = {1};".format( name, underlyingType ))
	...
	def genStruct( self, typeinfo, name, alias = None ):
		...
		for member in typeinfo.elem.findall( "member" ):
			...
			if self.currentFeature == "// VK_KHR_xlib_surface":
				memberType = re.sub( r"\bDisplay\b", "X11Library.Display", memberType )
				memberType = re.sub( r"\bWindow\b", "X11Library.Window", memberType )
			memberName = convertName( member.find( "name" ).text )
	...
	def genGroup( self, groupinfo, groupName, alias = None ):
		...
		enumBase = " : ulong" if groupElem.get( "bitwidth" ) == "64" else ""
		body = "\nenum " + groupName + enumBase + " {\n"
		...
		for elem in groupElem.findall( 'enum' ):
			...
			if isEnum and elem.get( 'extends' ) is None and numVal is not None:
				if minName is None:
	...
	def genCmd( self, cmdinfo, name, alias = None ):
		...
		joinedParams = ", ".join( convertTypeConst( getFullType( param, self.opaqueStruct ).strip()) + " " + convertName( param.find( "name" ).text ) for param in params )
		if self.currentFeature == "// VK_KHR_xlib_surface":
			joinedParams = re.sub( r"\bDisplay\b", "X11Library.Display", joinedParams )
			joinedParams = re.sub( r"\bVisualID\b", "X11Library.VisualID", joinedParams )
			joinedParams = re.sub( r"\bWindow\b", "X11Library.Window", joinedParams )
		...
		if name != "vkGetDeviceProcAddr" and getFullType( params[ 0 ] ) in { "VkDevice", "VkQueue", "VkCommandBuffer" }:
			...
			joinedArgs = ""
			if len( params[1:] ):
				joinedArgs = ", " + ", ".join( convertName( param.find( "name" ).text ) for param in params[1:] )
			joinedParams = ", ".join( convertTypeConst( getFullType( param, self.opaqueStruct ).strip()) + " " + convertName( param.find( "name" ).text ) for param in params[1:] )
...
if __name__ == "__main__":
	...
	vkxml = path.join( args.registry, "vk.xml" )

	gen = DGenerator()
	gen_opts = DGeneratorOptions(
			filename = args.outfolder,
			conventions = VulkanConventions(),
			apiname = "vulkan",
			versions = r"VK_VERSION_1_[0-3]",
			emitversions = r"VK_VERSION_1_[0-3]",
		packagePrefix = args.packagePrefix,
		namePrefix = args.namePrefix,
		genFuncPointers  = True,
		#defaultExtensions = "defaultExtensions",
			addExtensions = r"VK_(KHR_(surface|swapchain|xlib_surface|win32_surface)|EXT_debug_utils)$",
			emitExtensions = r"VK_(KHR_(surface|swapchain|xlib_surface|win32_surface)|EXT_debug_utils)$",
			#removeExtensions = None#r"VK_KHR_.*_surface$"
		)
	reg = Registry(gen, gen_opts)
